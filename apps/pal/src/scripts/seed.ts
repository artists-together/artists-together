...
import { getChannel, getRole } from "../lib/client.ts"
import { CHANNEL, ROLE } from "../lib/constants.ts"
...
async function bootstrapRulesChannel() {
  const [rulesChannel, introductionsChannel] = await Promise.all([
    getChannel(CHANNEL.RULES_N_FAQ),
    getChannel(CHANNEL.INTRODUCTIONS),
  ])

  if (!rulesChannel.isSendable()) {
    throw Error("Expected rules channel to be sendable", {
      cause: CHANNEL.RULES_N_FAQ,
    })
  }
  ...
  await rulesChannel.send({
    files: [
      new AttachmentBuilder(
        readFileSync(`./public/images/banners/banner-about-rules.png`),
      ),
    ],
  })
  ...
  await rulesChannel.send({
    files: [
      new AttachmentBuilder(
        readFileSync(`./public/images/banners/banner-pal.png`),
      ),
    ],
  })
  ...
  await rulesChannel.send({
    files: [
      new AttachmentBuilder(
        readFileSync(`./public/images/banners/banner-roles.png`),
      ),
    ],
  })
  ...
  const [roleModerator, roleVerified, roleUnverified, roleWeb, rolePal] =
    await Promise.all([
      getRole(ROLE.MOD),
      getRole(ROLE.VERIFIED),
      getRole(ROLE.UNVERIFIED),
      getRole(ROLE.WEB),
      getRole(ROLE.PAL),
    ])
